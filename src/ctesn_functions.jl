...
function nonlinear_mapping(sys, busCap, lb, ub, trainSize, totalPower, rSol, stateIndex, simStep)
    ...
    function get_W_rbf(p)
        ...
        UB=vec(maximum(R, dims=2));
        LB=vec(minimum(R, dims=2));

        Input = [R[:,i] for i in 1:size(R,2)];
        Output = [S[:,i] for i in 1:size(S,2)];
        
        surr = RadialBasis(Input, Output, LB, UB, rad = linearRadial)
        beta = surr.coeff
        Wout = (svd(R') \ S')' # calculate read-out matrix
        beta, surr, R, S
    end

    res = pmap(x -> get_W_rbf(trainSamples[:,x]), 1:trainSize) # Distribute computaiton among all available workers
    Woutparams = [trainSamples[:,i] for i = 1:size(trainSamples, 2)] # Distribute computaiton among all available workers
    Wouts = map(x -> x[1], res) # get read-out matrices from returned res object
    surr=res[1][2]
...
function nonlinear_predict(p, betaSurr, Surr, rSol, simStep, N, resSize)
    weights=reshape(betaSurr(p), size(Surr.coeff)[1], size(Surr.coeff)[2])
    Surr.coeff = weights
    pred = reduce(hcat, Surr.(rSol(simStep).u))
    pred
end